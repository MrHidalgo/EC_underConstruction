(function (angular) {
    ...
    var controller = function ($scope, $state, $filter, submissionService, modalService, $localStorage, $translate /*statusMessage*/, $rootScope) {
        ...
        $scope.remove = remove;
        $scope.filter = filter;
        $scope.loadRegistrationTypes = loadRegistrationTypes;
        $scope.reset = reset;
        ...
        function getSubmissionHistory() {
            ...
            var registrationTypes = angular.isNullOrUndefined($scope.filterParams.registrationTypes)
                ? []
                : _($scope.filterParams.registrationTypes).pluck("Id");
            ...
            var approvals = angular.isNullOrUndefined($scope.filterParams.approvals)
                ? []
                : _($scope.filterParams.approvals).pluck("Name");
            // console.log(approvals);
            submissionService
                .listCollege(null, $scope.pageNumber, startDate, endDate, registrationTypes)
            ...
            submissionService
                ...
                .then(function (response) {
                    ...
                    if (!response) {
                     ...
                     console.log(response);
                     $scope.filterParams.registrationTypesOptions = response;
                     console.log(response);
                });
        }

        function reset() {
            $scope.filterParams.registrationTypes = null;
    ...
    angular
        ...
        .controller("studentcollege-submissionhistory.controller", controller);
